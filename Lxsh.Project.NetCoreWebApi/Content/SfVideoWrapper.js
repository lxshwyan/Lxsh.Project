...
; +function ($) {
    ...
    * ui-choose插件
    */
    $.fn.sfvideo = function (options) {
        var _this = $(this),
            _num = _this.length;
        // 当要实例的对象只有一个时，直接实例化返回对象；
        if (_num === 1) {
            return new SfVideo(_this, options);
        };

        alert("视频控件只允许同时存在一个实例");
        return null;
        // 当要实例的对象有多个时，循环实例化，不返回对象；
        //if (_num > 1) {
        //    _this.each(function (index, el) {
        //        new SfVideo($(el), options);
        //    })
        //}
        // 当元素个数为0时，不执行实例化。
    };
    ...
    */
    ...
    SfVideo.prototype = {
        ...
        disabled: function () {
            ...
            //this.el.find('option').prop('disabled', 'disabled');
            //_this._wrap.prop('disabled') = 'disabled';
            //this._wrap.prop('disabled', 'disabled');
            //this._wrap.attr('disabled', 'disabled');
            //this._wrap.disabled();
            //this._wrap.disabled('disabled');
            //this.disabled();
            return this;
            // _disabled = _this.prop('disabled') ? ' disabled' : '';
        },
        ...
        ,
        destory: function () {
            this.el.data(this._opt.dataKey, null);
            for (var ii = 0; ii < this._wrap.length; ii++) {
                this._wrap[ii].remove();
            }
        }
    };
}(jQuery);
